...
export default function GenerationView({ onComplete }) {
  ...
  const removeFile = (indexToRemove) => {
    setFileTexts(prev => prev.filter((_, index) => index !== indexToRemove));
            ...
            {uploadedFiles.length > 0 && (
              <div className="mt-4 space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                {uploadedFiles.map((file, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm animate-in fade-in slide-in-from-bottom-2">
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div className="p-2 bg-white dark:bg-slate-900 rounded-lg text-primary shadow-sm border border-slate-100 dark:border-slate-800">
                        <FileIcon size={16} />
                      </div>
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate max-w-[200px]" title={file.name}>{file.name}</span>
                    </div>
                    <button 
